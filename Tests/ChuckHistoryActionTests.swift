...
final class ChuckHistoryActionTests: XCTestCase {
    ...
    func testTransferCashIn() throws {
        ...
        let actual = try imp.decodeDelimitedRows(delimitedRows: delimitedRows,
                                                 ...
                                                 timeZone: tzNewYork,
                                                 rejectedRows: &rr)
        let expected: [AllocRowed.DecodedRow] = [["txnShareCount": 101_000.00, "txnAccountID": "1", "txnAction": MTransaction.Action.transfer, "txnTransactedAt": timestamp1, "txnSharePrice": 1.0]]
        XCTAssertEqual(expected, actual)
    }
