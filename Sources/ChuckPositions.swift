...
struct ChuckPositions {
    ...
    static func holding(_ accountID: String, _ row: AllocRowed.RawRow, rejectedRows _: inout [AllocRowed.RawRow]) -> AllocRowed.DecodedRow? {
        // NOTE: 'Symbol' may be "Cash & Cash Investments" or "Account Total"
        guard let rawSymbol = getRawSymbol(row) else { return nil }

        var netSymbol: String?
        var shareBasis: Double?
        var netShareCount: Double?
